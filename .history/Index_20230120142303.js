...
async function run() {
  ...
  app.put("/user/admin/:email",VERIFYJWT, async (req, res) => {
    ...
    const requester = red.decoded.email;
    const requesterdecoded = await BookUserCollection.findOne({email : requester})
    if(requesterdecoded.role === 'admin'){
   ...
   return  res.send(result);
      ...
      return res.status(403).send({message : "Forbiden acc"})
    }
